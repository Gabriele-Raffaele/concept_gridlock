...
class LaneModule(pl.LightningModule):
    ...
    def __init__(self, model, bs, multitask="multitask", dataset="comma", time_horizon=1, ground_truth="desired", intervention=False, dataset_path=None, dataset_fraction=1.0):
    ...
    def calculate_loss(self, logits, angle, distance):
        ...
        if self.multitask == "multitask":
            ...
            self.log_dict({"train_loss_angle": loss_angle}, on_epoch=True, batch_size=self.bs,sync_dist=False)
            self.log_dict({"train_loss_distance": loss_distance}, on_epoch=True, batch_size=self.bs, sync_dist=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.multitask == "multitask":
            ...
            self.log_dict({"val_loss_dist": loss_dist}, on_epoch=True, batch_size=self.bs, sync_dist=False)
            self.log_dict({"val_loss_angle": loss_angle}, on_epoch=True, batch_size=self.bs, sync_dist=False)
        self.log_dict({"train_loss": loss}, on_epoch=True, batch_size=self.bs, sync_dist=False)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.multitask == "multitask":
            ...
            self.log_dict({"val_loss_dist": loss_dist}, on_epoch=True, batch_size=self.bs, sync_dist=False)
            self.log_dict({"val_loss_angle": loss_angle}, on_epoch=True, batch_size=self.bs, sync_dist=False)
        self.log_dict({"val_loss": loss}, on_epoch=True, batch_size=self.bs, sync_dist=False)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.time_horizon > 1:
            ...
            self.log_dict({"test_loss": loss}, on_epoch=True, batch_size=self.bs, sync_dist=False)
            return loss
        ...
        if self.multitask == "multitask":
            ...
            param_angle, param_dist = 0.3, 0.7
            loss = (param_angle * loss_angle) + (param_dist * loss_dist)
            self.log_dict({"test_loss_dist": loss_dist}, on_epoch=True, batch_size=self.bs, sync_dist=False)
            self.log_dict({"test_loss_angle": loss_angle}, on_epoch=True, batch_size=self.bs, sync_dist=False)
        self.log_dict({"test_loss": loss}, on_epoch=True, batch_size=self.bs, sync_dist=False)
        return loss
    #------------------------------------------------------------
    ...
    def train_epoch_end(self, outputs):
        ...
        self.log_dict({"train_loss_accumulated": losses }, batch_size=self.bs, sync_dist=False)

    def validation_epoch_end(self, outputs):
        losses = torch.mean(torch.stack([x for x in outputs]))
        self.log_dict({"val_loss_accumulated": losses }, batch_size=self.bs, sync_dist=False)

    def test_epoch_end(self, outputs):
        losses = torch.mean(torch.stack([x for x in outputs]))
        self.log_dict({"test_loss_accumulated": losses }, batch_size=self.bs, sync_dist=False)
